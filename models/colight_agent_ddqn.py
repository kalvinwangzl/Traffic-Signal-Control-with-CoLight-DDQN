...
class CoLightAgent(Agent):
    ...
    def prepare_Xs_Y(self, memory):
        ...
        next_state_qvalues_current = self.q_network([_next_state2, _adjs2])  
        next_state_qvalues_target = self.q_network_bar([_next_state2, _adjs2]) 
        # next_state_qvalues = self.q_network_bar([_next_state2, _adjs2])
        # [batch, agent, num_actions]
        final_target = np.copy(target)
        for i in range(slice_size):
            for j in range(self.num_agents):
                next_action = np.argmax(next_state_qvalues_current[i, j])
                final_target[i, j, _action[j][i]] = _reward[j][i] / self.dic_agent_conf["NORMAL_FACTOR"] + \
                                            self.dic_agent_conf["GAMMA"] * next_state_qvalues_target[i, j, next_action]
                # final_target[i, j, _action[j][i]] = _reward[j][i] / self.dic_agent_conf["NORMAL_FACTOR"] + \
    ...
    def soft_update_target_network(self, tau=0.01):
        for target_var, local_var in zip(self.q_network_bar.trainable_variables, self.q_network.trainable_variables):
